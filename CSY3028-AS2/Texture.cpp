...
Texture::Texture(char* ps, int w, int h)
{
	pixels = ps;
	width = w;
	height = h;
...
Texture::Texture(const char* filename)
{
	...
	auto_array<char> px(new char[size]);
	input.seekg(dataOffset, std::ios_base::beg);
	input.read(px.get(), size);

	//Get the data into the right format
	auto_array<char> px2(new char[width * height * 3]);
	for (int y = 0; y < height; y++) {
		for (int x = 0; x < width; x++) {
			for (int c = 0; c < 3; c++) {
				px2[3 * (width * y + x) + c] =
					px[bytesPerRow * y + 3 * x + (2 - c)];
			}
		}
	}
